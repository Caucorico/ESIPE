...
public class CmdLineParser {
    ...
    private interface ParameterRetrievalStrategy {
        List<String> retrieve(PaintOption option, List<String> next) throws ParseException;
    }

    public static final ParameterRetrievalStrategy STANDARD = (option, next) -> {

        for ( var k = 0 ; k < option.paramNumber ; k++ ) {
            var param = next.get(k);
            if ( param.startsWith("-") ) {
                throw new ParseException("Not enough argument, the param " + param + " is encounter before the end of the arguments number", 0);
            }
        }

        return new ArrayList<>(next.subList(0, option.paramNumber));
    };

    public static final ParameterRetrievalStrategy RELAXED = (option, next) -> {
        var k = 0;

        for ( k = 0 ; k < option.paramNumber ; k++ ) {
            var param = next.get(k);
            if ( param.startsWith("-") ) {
                break;
            }
        ...
        return new ArrayList<>(next.subList(0, k));
    };

    public static final ParameterRetrievalStrategy OLDSCHOOL = (option, next) -> {
        return new ArrayList<>(next.subList(0, option.paramNumber));
    };

    public CmdLineParser() {
        OptionsObserver observer = new RequireObserver();
        OptionsObserver observer2 = new ConflictObserver();
    ...
    public List<String> process(String[] arguments, ParameterRetrievalStrategy parameterRetrievalStrategy) throws ParseException {
        Objects.requireNonNull(arguments);
        Objects.requireNonNull(parameterRetrievalStrategy);
        ...
        while ( i < arguments.length ) {
            ...
            if ( existing.isPresent() ) {
                ...
                List<String> params = new ArrayList<>(option.paramNumber);
                params.addAll(Arrays.asList(arguments).subList(i+1, arguments.length));

                params = parameterRetrievalStrategy.retrieve(existing.get(), params);
    ...
    public List<String> process(String[] arguments) throws ParseException {
        Objects.requireNonNull(arguments);
        return this.process(arguments, STANDARD);
    }
